---
name: herdr
description: "Control herdr from inside it. Manage workspaces and tabs, split panes, spawn agents, read output, and wait for state changes — all via CLI commands that talk to the running herdr instance over a local unix socket. Use when running inside herdr (HERDR_ENV=1). Also covers agent detection and REMOTE agents: use whenever an agent behind ssh/tmux or on a remote herdr server isn't showing in the agents sidebar, status isn't updating, stale/orphan agent names linger, or when working with pane report-agent / release-agent / agent rename / agent explain, the Remote Agent Watch plugin, or the sshl/herdrl wrappers."
---
...
## remote agents (ssh / tmux / remote herdr)

agents running behind ssh never appear in the sidebar on their own: the
agent label is gated on the pane's **local** process tree, which only shows
`ssh`. before debugging "why isn't this agent detected", or doing anything
with `pane report-agent`, `release-agent`, `agent rename`, `agent explain`,
remote herdr servers, or the sshl/herdrl wrappers, read
[references/remote-agents.md](references/remote-agents.md) — it holds the
verified detection model and this machine's tooling. the short version:

- this machine has the **Remote Agent Watch** plugin
  (`~/work/herdr-remote-agent-watch/watch.sh`) — scrape mode for ssh/tmux
  panes, bridge mode (`--remote <host> <sock>`) when the remote box runs
  its own herdr. `sshl` and `herdrl` arm it automatically.
- externally reported state is reporter-owned (screen rules won't update
  it), `report-agent` can't say `done`, and a released agent with a
  leftover `agent rename` name lingers in the sidebar — clear the name too.
- `agent explain --agent claude` defaults to idle when nothing matches
  (`rule: none`) — check the rule line, and feed it only the screen bottom
  (`pane read --source visible | tail -12`).
